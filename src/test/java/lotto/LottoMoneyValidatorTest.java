...
class LottoMoneyValidatorTest {
    ...
    void 보너스_번호가_음수의_값으로_들어오면_예외_발생(String money) {
        assertThrowWithMessageByChangeBonusNumber(money, "양수의 금액을 입력해주세요");
    }

    @ParameterizedTest
    @ValueSource(strings = {"abcd", "금액", "one", "!@#", "1000원", "천원", "one thousand", "1234,56", "", " "})
    void 보너스_숫자가_숫자가_아닌_다른_값을_입력하면_예외_발생(String money) {
        assertThrowWithMessageByChangeBonusNumber(money, "정상적인 금액을 입력해주세요");
    }

    @ParameterizedTest
    @ValueSource(strings = {"1.1", "45.1", "23.56"})
    void 보너스_값을_소수_값으로_들어오면_예외_발생(String money) {
        assertThrowWithMessageByChangeBonusNumber(money, "소수점 값은 입력할 수 없습니다");
    }
}
